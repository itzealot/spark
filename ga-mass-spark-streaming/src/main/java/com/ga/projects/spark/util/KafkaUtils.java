...
public class KafkaUtils {
	...
	public static TreeMap<Integer, PartitionMetadata> findLeader(String brokerHost, int a_port, String a_topic) throws Exception {
		...
		try {
			...
			throw new Exception("Error communicating with Broker [" + brokerHost + "] to find Leader for [" + a_topic + ", ]", e);
		} finally {
			if (consumer != null) {
				consumer.close();
			}
	...
	private static Map<Integer, Long> getOffset(String topic, List<String> kafkaHosts, List<Integer> kafkaPorts, boolean isLast) throws Exception {
		...
		for (int i = 0, size = kafkaHosts.size(); i < size; i++) {
			...
			try {
				...
				throw new Exception("topic(" + topic + "), kafkaHost(" + host + "), kafkaPort(" + port + "), Kafka getOffset error! isLast: " + isLast, e);
			}
	...
	private static Map<Integer, Long> getOffset(String topic, String kafkaHost, int kafkaPort, boolean isLast) throws Exception {
		...
		try {
			...
			throw new Exception("topic(" + topic + "), kafkaHost(" + kafkaHost + "), kafkaPort(" + kafkaPort + "), Kafka findLeader error! isLast: " + isLast, e);
		}
		...
		for(Entry<Integer, PartitionMetadata> entry : metadatas.entrySet()) {
			...
			try {
				...
				if (isLast) {
					// 获取最新偏移
					offset = KafkaUtils.getOffset(consumer, topic, partition, kafka.api.OffsetRequest.LatestTime(), clientName);
				} else {
					// 获取最早偏移
					offset = KafkaUtils.getOffset(consumer, topic, partition, kafka.api.OffsetRequest.EarliestTime(), clientName);
				}
				partionAndOffset.put(partition, offset);
			} catch (Exception e) {
				throw new Exception("topic(" + topic + "), kafkaHost(" + kafkaHost + "), kafkaPort(" + kafkaPort + "), Kafka fetch getOffset error! isLast: " + isLast, e);
			} finally {
				if(consumer!=null){
					consumer.close();	
				}
